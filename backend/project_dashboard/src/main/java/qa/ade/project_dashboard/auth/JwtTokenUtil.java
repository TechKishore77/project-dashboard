package qa.ade.project_dashboard.auth;
...
public class JwtTokenUtil implements Serializable {
    ...
    public JwtTokenPair generateAndSaveTokenPair(UserDetails userDetails) {
        final String accessToken = generateToken(userDetails);
        ...
        if (tokenService.addTokenPair(tokenPair, userDetails.getUsername(), Timestamp.from(getExpirationDateFromToken(accessToken).toInstant()), Timestamp.from(getExpirationDateFromToken(refreshToken).toInstant())))
            return tokenPair;
        return null;
    }
}
